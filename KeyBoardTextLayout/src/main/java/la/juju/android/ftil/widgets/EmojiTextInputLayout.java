...
import la.juju.android.ftil.adapters.TextPagerAdapter;
 ...
 */
public class EmojiTextInputLayout extends LinearLayout {

  private static final String TAG = EmojiTextInputLayout.class.getSimpleName();

  public static final int PAGE_MAX_LINE_NUM = 4; // 每页的最大行号

  public static final int PAGE_MAX_COLUMN_COUNT = 6; // 每页的最大列数
  ...
  private LayoutParams mFaceTextContainerLayoutParams; // 颜文字容器的“LayoutParams”参数

  private FaceTextProvider mFaceTextProvider; // “颜文字source”接口

  private OnTextClickListener itemClickListener; // "颜文字"点击事件回调接口

  public EmojiTextInputLayout(Context context) {
    this(context, null);
  }

  public EmojiTextInputLayout(Context context, AttributeSet attrs) {
    this(context, attrs, R.attr.FaceTextInputLayoutStyle);
  }

  public EmojiTextInputLayout(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  private void applyXMLAttributes(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    TypedArray typedArray =
        getContext().obtainStyledAttributes(attrs, R.styleable.FaceTextInputLayout, defStyleAttr, defStyleRes);

    // 默认"indicator"间距 10dp
    int indicatorSpacing = typedArray.getDimensionPixelSize(R.styleable.FaceTextInputLayout_indicatorSpacing,
        DensityUtil.dip2px(getContext(), 10));
    mDotViewLayout.setIndicatorSpacing(indicatorSpacing);

    // 获取被选中的"indicator"图片资源 Id
    int selectedIndicatorResId = typedArray.getResourceId(R.styleable.FaceTextInputLayout_selectedSrc, -1);
    mDotViewLayout.setSelectedIndicatorResId(selectedIndicatorResId);

    // 获取未选中的"indicator" 图片资源 Id
    int unSelectedIndicatorResId = typedArray.getResourceId(R.styleable.FaceTextInputLayout_unselectedSrc, -1);
    mDotViewLayout.setUnSelectedIndicatorResId(unSelectedIndicatorResId);

    mFaceTextViewLeftMargin = typedArray.getResourceId(R.styleable.FaceTextInputLayout_faceTextViewLeftMargin,
        DensityUtil.dip2px(getContext(), 2));

    mFaceTextViewRightMargin = typedArray.getResourceId(R.styleable.FaceTextInputLayout_faceTextViewLeftMargin,
        DensityUtil.dip2px(getContext(), 2));
   ...
   * 生成所有“颜文字”页面
   */
  private List<View> generateAllPage() {
  ...
  private RecyclerView generateEachPage(List<List<FaceText>> faceTextList) {
    ...
    faceTextInputLineAdapter.setOnFaceTextClickListener(itemClickListener);
   ...
   */
  private LayoutParams generateFaceTextContainerLayoutParams() {
    if (mFaceTextContainerLayoutParams == null) {
      mFaceTextContainerLayoutParams = new LayoutParams(0, calculateFaceTextViewHeight(), 1.0F);
   ...
   * 设置“颜文字”点击事件回调
   */
  public void setOnFaceTextClickListener(OnTextClickListener onFaceTextClickListener) {
    itemClickListener = onFaceTextClickListener;
  }
   ...
   */
  public void render() {
    List<View> allPageList = generateAllPage();
    mTextPagerAdapter.setFaceTextInputPageList(allPageList);
