...
public class InputMethodUtils {
    ...
    if (currentFocusView != null) {
      ...
      if (token != null) {
        InputMethodManager im =
            (InputMethodManager) currentFocusView.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
  ...
  public static int getSupportSoftKeyboardHeight(Activity activity) {
    ...
    if (softKeyboardHeight == 0) {
      SharedPreferences sharedPreferences =
          activity.getSharedPreferences(NAME_PREF_SOFT_KEYBOARD, Context.MODE_PRIVATE);
      softKeyboardHeight =
          sharedPreferences.getInt(KEY_PREF_SOFT_KEYBOARD_HEIGHT, dip2px(activity, DEFAULT_SOFT_KEYBOARD_HEIGHT));
    }
   ...
   * @return 虚拟按键的高度
   */
  @TargetApi (Build.VERSION_CODES.JELLY_BEAN_MR1) private static int getNavigationBarHeight(Context context) {
