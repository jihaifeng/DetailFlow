...
import com.meechao.detailflow.utils.MeechaoDataUtils;
...
public class PageScrollView extends HorizontalScrollView {
  ...
  public void setData(List<String> data, TagStyle tagStyle) {
    if (origHeight != 0) {
      // 还原初始高度
    ...
    for (int i = 0; i < data.size(); i++) {
      if (i < childCount) {
        ...
        updateTag(tvItem, data.get(i), tagStyle);
      } else {
        addTag(data.get(i), tagStyle);
      }
    }
    while (flowLayout.getChildCount() > data.size()) {
      flowLayout.removeViewAt(data.size());
    }
  }
  ...
  public CheckedTextView addTag(String text, TagStyle tagStyle, int index) {
    ...
    PageFlowLayout.LayoutParams lp =
        new PageFlowLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
    if (tagStyle != null) {
    ...
    tvItem.setText(MeechaoDataUtils.covTopicVal(text));
    tvItem.setTag(MeechaoDataUtils.covTopicVal(text));
  ...
  private OnClickListener clickListener = new OnClickListener() {
    @Override public void onClick(View view) {
      ...
      if (tagClickListener != null) {
        tagClickListener.onClick(view, null == obj ? "" : obj.toString());
      }
    }
  };
