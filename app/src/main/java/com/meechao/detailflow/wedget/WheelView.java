...
public class WheelView extends View {
   ...
   * 线的默认宽度
   */
  private float lineHeight;//px
  /**
   * 默认字体
   */
  private float normalFont;//px
  /**
   * 选中的时候字体
   */
  private float selectedFont;//px
  /**
   * 单元格高度
   */
  private float unitHeight;//px
  /**
   * 显示多少个内容
   */
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    unitHeight = (int) attribute.getDimension(R.styleable.WheelView_unitHeight, dp2px(35));
    //按理说应该是奇数，但是此次不做判断，使用者有义务设置正确
    itemNumber = attribute.getInt(R.styleable.WheelView_itemNumber, 5);

    normalFont = attribute.getDimension(R.styleable.WheelView_normalTextSize, sp2px(18));
    selectedFont = attribute.getDimension(R.styleable.WheelView_selectedTextSize, sp2px(22));
    normalColor = attribute.getColor(R.styleable.WheelView_normalTextColor, Color.argb(255, 51, 51, 51));
    selectedColor = attribute.getColor(R.styleable.WheelView_selectedTextColor, Color.argb(255, 51, 51, 51));

    lineColor = attribute.getColor(R.styleable.WheelView_lineColor, Color.argb(255, 215, 215, 215));
    lineHeight = attribute.getDimension(R.styleable.WheelView_lineHeight, 1);
    ...
    isCyclic = attribute.getBoolean(R.styleable.WheelView_isCyclic, false);

    maskDarkColor = attribute.getColor(R.styleable.WheelView_maskDarkColor, DEFAULT_MASK_DARK_COLOR);
    maskLightColor = attribute.getColor(R.styleable.WheelView_maskLightColor, DEFAULT_MASK_LIGHT_COLOR);
  ...
  private void drawMask(Canvas canvas) {
    if (mastPaint == null) {
      mastPaint = new Paint();
      linearGradientUp = new LinearGradient(0, 0, 0, unitHeight, maskDarkColor, maskLightColor, Shader.TileMode.CLAMP);
      linearGradientDown =
          new LinearGradient(0, controlHeight - unitHeight, 0, controlHeight, maskLightColor, maskDarkColor,
    ...
    canvas.drawRect(0, controlHeight - itemNumber / 2 * unitHeight, controlWidth, controlHeight, mastPaint);
  }
    ...
    if (onSelectListener != null) {
      callbackHandler.post(new Runnable() {
        @Override public void run() {
          onSelectListener.endSelect(toShowItem.index, toShowItem.getItemText());
        }
      });
    }
  }
  ...
  private void findItemsToShow() {
    ...
    if (onSelectListener != null && toShowItems[itemNumber / 2] != null) {
      callbackHandler.post(new Runnable() {
        @Override public void run() {
          onSelectListener.selecting(toShowItems[itemNumber / 2].index, toShowItems[itemNumber / 2].getItemText());
        }
      });
    }
  }
  ...
  private class ItemObject {
    ...
    public void drawSelf(Canvas canvas, int containerWidth) {
      ...
      canvas.drawText(itemText, ((float) x + (float) controlWidth / 2f - (float) textRect.width() / 2f),
          ((float) (y + move) + unitHeight / 2 + (float) textRect.height() / 2f) - baseline, textPaint);
    }
    ...
    public synchronized boolean isInView() {
      //            if (y + move > controlHeight || ((float)y + (float)move + (float)unitHeight / 2 + (float)textRect
      // .height() / 2f) < 0)
      //放宽判断的条件，否则就不能再onDraw的开头执行，而要到textRect测量完成才能执行
      return !(y + move > controlHeight || ((float) y + (float) move + (float) unitHeight) < 0);
    }
    ...
    public synchronized boolean selected() {
      ...
      if ((y + move >= itemNumber / 2 * unitHeight - unitHeight / 2 + (float) textRect.height() / 2) && (y + move
