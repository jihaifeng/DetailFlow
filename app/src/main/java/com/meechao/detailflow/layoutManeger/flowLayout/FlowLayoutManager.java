...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
  ...
  private void fillLayout(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    Rect displayFrame =
        new Rect(getPaddingLeft(), getPaddingTop() + verticalScrollOffset, getWidth() - getPaddingRight(),
    ...
    for (int j = 0; j < lineRows.size(); j++) {
      ...
      if (lineTop < displayFrame.bottom && displayFrame.top < lineBottom) {
        ...
        for (int i = 0; i < views.size(); i++) {
          ...
          //将这个item布局出来
          layoutDecoratedWithMargins(scrap, frame.left, frame.top - verticalScrollOffset, frame.right,
